...
####
### This script is to extract Uniq blast results (three donors) and statistic their information.
### requirement: 1
#####1
...
if (!defined $opts{i} || !defined $opts{o} || !defined $opts{q} ) {
	...
	Request Parameters for three donor deduplication:
	-i Insertion with detected three donor
	-q The quality information of each read
	-o Output of unique insertion that only detected with three donor, this including the clusting file and unique insertion event file
...
my %num; my %match; my %hash; my %fiden; my %Rcount; my %inf;
open I,"$input" or die $!;
my $n=1;
while (<I>){
	chomp;
	s/\r//g;
	my ($cov,$len,$qid,$p1id,$p1qstart,$p1qend,$p1pstart,$p1pend,$p1match,$p1ident,$p2id,$p2qstart,$p2qend,$p2pstart,$p2pend,$p2match,$p2ident,$p3id,$p3qstart,$p3qend,$p3pstart,$p3pend,$p3match,$p3ident)=split/\s+/,$_;
	$fiden{$qid}=($p1ident+$p2ident+$p3ident)/3;
	next if ($qid eq "qID");
	my $information=$_;
	$Rcount{$qid}=$cov;
...
# 	my $nend=$Rqend;
	...
	my $q=$n-1;
	foreach my $i (1..$q){
		if ($p1id eq $hash{$i}->{p1id} && (abs($p1qstart-$hash{$i}->{p1qstart}) <=$gapA) && (abs($p1qend-$hash{$i}->{p1qend}) <=$gapA)  && (abs($p1pstart-$hash{$i}->{p1pstart}) <=$gapB) && (abs($p1pend-$hash{$i}->{p1pend}) <=$gapB) && $p2id eq $hash{$i}->{p2id} && (abs($p2qstart-$hash{$i}->{p2qstart}) <=$gapA) && (abs($p2qend-$hash{$i}->{p2qend}) <=$gapA)  && (abs($p2pstart-$hash{$i}->{p2pstart}) <=$gapB) && (abs($p2pend-$hash{$i}->{p2pend}) <=$gapB)&& ($p3id eq $hash{$i}->{p3id}) && (abs($p3qstart-$hash{$i}->{p3qstart}) <=$gapA) && (abs($p3qend-$hash{$i}->{p3qend}) <=$gapA)  && (abs($p3pstart-$hash{$i}->{p3pstart}) <=$gapB) && (abs($p3pend-$hash{$i}->{p3pend}) <=$gapB) ){
...
foreach my $num(keys %hash){
	...
	print CLT "SecondFinalThree\tTID$num\t$FRcount\t$RepRead\t$qual{$RepRead}\t$fiden{$RepRead}\t$fid\t$fqual\t$fiden\t$fcov\n";
