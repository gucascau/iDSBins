...
my $n=0;
print OUT "Case\tNID\tGenotype\tWholeinsertion\tInsertionSequence\tInsertionSize\tDonorNumber\tIdentity\tTotalCoverage\tBitscore\tPvalue\tRead\tFupstreamq\tFdownstreamq\tRupstreamq\tRdownstreamq\tDonorChromosome\tDonorStart\tDonorEnd\tStrand\tAnnotationFeature\tDistanceToFeature\tDistanceDescript\tAlterFeature\tJunction_5\tJunction_3\n";
while(<B>){
	chomp;
	my ($cov,$id,$chr,$iden,$match,$start,$end,$total,$Rstart,$Rend,$fcov)=(split/\t/,$_)[0,1,2,3,4,7,8,9,10,11,15];
	...
	my $startF= ($startA<=50)?$startA:44;
	
	my $endF=(($lengthall-$endC)<=60)?$endC:($lengthall-50);
	my $length=$endF-$startF+1;
	my $insertion=substr $string{$id},$startA, $length;
