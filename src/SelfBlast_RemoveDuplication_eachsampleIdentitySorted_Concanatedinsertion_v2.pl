...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"f:s","o:s","i:s","b:s","h:s","t:s","g:s","m:s","c:s","r:s");
print "*************\n*$version*\n*************\n";
if (!defined $opts{f} ||!defined $opts{o} ||!defined $opts{i}||!defined $opts{b}|| defined $opts{h}||!defined $opts{r}) {
	die "************************************************************************
	Usage: $0.pl -f forward -r reverse -i Read with evaluation -b SelfBlast -o Output of Quality Control Reads
	
	Request Parameters:
	-b Blast Results (Self Blast results))
	-i The evaluation of each inserted reads(quality, identity)
	-f UnAssembled forward reads (After first round of deduplication base)
	-r UnAssembled reverse reads
...
### 		unassembled fastq files for large insertion events
######################################################################################################################################
...
open FOUF,">$output.FUassuniq.forward.fasta" or die $!;
open FOUFQ,">$output.FUassuniq.forward.fastq" or die $!;
open FOUR,">$output.FUassuniq.reverse.fasta" or die $!;
open FOURQ,">$output.FUassuniq.reverse.fastq" or die $!;
#open ROUT,">$output.Funiq.reverse.fastq" or die $!;
my $uncls=0;
foreach my $f (keys %Fhash){
	
	if (exists $uniq{$f}){
		print FOUF ">$f\n$Fhash{$f}\n";
		print FOUR ">$f\n$Rhash{$f}\n";
		
		print FOUFQ "$FhashQ{$f}";
		print FOURQ "$RhashQ{$f}";
		...
		print CLS "UnAssembly\tUndefined$uncls\t$Fcoverage\t$f\t$inf{$f}->{quality}\t$inf{$f}->{identity}\tNoClustered\t$inf{$f}->{identity}\t$inf{$f}->{quality}\tNoRank\n";
	}
