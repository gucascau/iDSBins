...
if (!defined $opts{a} || !defined $opts{b} || !defined $opts{o} || !defined $opts{t}|| !defined $opts{c} || !defined $opts{d} || !defined $opts{e}  ) {
	die "************************************************************************
	Usage: $0.pl -a AssembledFasta -b QualityFile -c FinalFourdonorInsertion	-d BLASThit -e BLASTAnnotation -t Sample name -o Output string
				-a Assembled fasta file (Test_detected_assemblysep.Insassembled.fasta)
				-b Reads with Quality results (Test_QC.Allquality.stat)
				-c Final unique Four donor insertions (Test_detected_second_threeassembly.uniq.txt, this contain the reads coverage) 
...
while(<B>){
	...
	my ($fcov,$id,$cov,$lengthall,$id2,$chrA,$startA,$endA,$RstartA,$RendA,$lengthA0,$identA,$chrB,$startB,$endB,$RstartB,$RendB,$lengthB0,$identB,$chrC,$startC,$endC,$RstartC,$RendC,$lengthC0,$identC,$chrD,$startD,$endD,$RstartD,$RendD,$lengthD0,$identD)=split/\t/,$_;
	...
	my $endF=(($lengthall-$endD)<=60)?$endD:($lengthall-50);
	my $length=$endF-$startF+1;
	my $insertion=substr $string{$id},($startA-1), $length;
	
	
	### Calculate the donor number of insertions

	my $ndonor=4;
	if ($startA >60 || ($startB-$endA) >20 ||  ($startC-$endB) >20 || ($startD-$endC) >20 ||($lengthall-$endD)>60){
		$ndonor="4orMore";
	...
	##### find the insertion of the third Part


	my $strandC=($RstartC<$RendC)?"+":"-";
	my $dstartC=($RstartC<$RendC)?$RstartC:$RendC;
	my $dendC=($RstartC<$RendC)?$RendC:$RstartC;
	
	
	my $lengthC=$endC-$startC+1;
	my $insertionC= substr $string{$id},($startC-1),$lengthC;

	my $partC=join ":",($chrC,$dstartC,$dendC);
	my ($EchrC,$EstartC,$EendC,$EannotationC,$distC)=split ":",$element{$partC};

	my $featureC;
	if ($distC != 0){
		$featureC="PROXIMITY";
	}elsif($dstartC>=$EstartC && $dendC<=$EendC){
		$featureC="ENTIRE";
	}else{
		$featureC="PARTIALLY";
	}
	
	##### find the insertion of the four Part


	my $strandD=($RstartD<$RendD)?"+":"-";
	my $dstartD=($RstartD<$RendD)?$RstartD:$RendD;
	my $dendD=($RstartD<$RendD)?$RendD:$RstartD;
	
	
	my $lengthD=$endD-$startD+1;
	my $insertionD= substr $string{$id},($startD-1),$lengthD;

	my $partD=join ":",($chrD,$dstartD,$dendD);
	my ($EchrD,$EstartD,$EendD,$EannotationD,$distD)=split ":",$element{$partD};

	my $featureD;
	if ($distD != 0){
		$featureD="PROXIMITY";
	}elsif($dstartD>=$EstartD && $dendD<=$EendD){
		$featureD="ENTIRE";
	}else{
		$featureD="PARTIALLY";
	}
	...
	my $chrall=join";",($chrA,$chrB,$chrC,$chrD);
	my $idenall=join ";",($identA,$identB,$identC,$identD);
	
	my $quality=$qual{$id};
	print INSSEP ">F$n.A\t$id\n$insertionA\n>T$n.B\t$id\n$insertionB\n>T$n.C\t$id\n$insertionC\t>T$n.D\t$id\n$insertionD\n";
	

	print INS ">$id\n$string{$id}\n";
	
	### print the overall element of insertion
	
	print OUT "Fo$n\t$id\t$sample\t$string{$id}\t$insertion\t$length\t$ndonor\t$idenall\t$fcov\t$quality\t$chrall\tNO\tNO\tNO\tNO\tNO\tNO\tNO\t$upjunction\t$downjunction\n";
	
	### print the first element of insertion #
	
	print OUT "Fo$n\.A\t$id\t$sample\t$string{$id}\t$insertionA\t$lengthA\tT1\t$identA\t$fcov\t$quality\t$chrA\t$dstartA\t$dendA\t$strandA\t$EannotationA\t$distA\t$featureA\tNO\tNO\tNO\n";
	
	### print the second element of insertion #
	print OUT "Fo$n\.B\t$id\t$sample\t$string{$id}\t$insertionB\t$lengthB\tT2\t$identB\t$fcov\t$quality\t$chrB\t$dstartB\t$dendB\t$strandB\t$EannotationB\t$distB\t$featureB\tNO\tNO\tNO\n";
	
	### print the third element of insertion #
	print OUT "Fo$n\.C\t$id\t$sample\t$string{$id}\t$insertionC\t$lengthC\tT3\t$identC\t$fcov\t$quality\t$chrC\t$dstartC\t$dendC\t$strandC\t$EannotationC\t$distC\t$featureC\tNO\tNO\tNO\n";
	
	### print the four element of insertion
	print OUT "Fo$n\.D\t$id\t$sample\t$string{$id}\t$insertionD\t$lengthD\tT4\t$identD\t$fcov\t$quality\t$chrD\t$dstartD\t$dendD\t$strandD\t$EannotationD\t$distD\t$featureD\tNO\tNO\tNO\n";
