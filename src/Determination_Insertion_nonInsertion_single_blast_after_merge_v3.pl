...
use Getopt::Long;
my %opts;
GetOptions(\%opts,"i:s","o:s","q:s","f:s","h:s","j:s","m:s","e:s","n:s","t:s","c:s");
print "*************\n*$version*\n*************\n";
if (!defined $opts{i} || !defined $opts{o} || !defined $opts{q} || !defined $opts{f} || defined $opts{h}  || !defined $opts{j}) {
	die "************************************************************************
	Usage: $0.pl -i $0 -q Read_Quality -f Assembled_fasta -o Blast_categories
	...
	-o Output of different categories of the read
...
while (<I>) {
	...
	###### confident non-insertion, here might contain the short indels. Due to the concentration of large insertion, we will take these results into consideration.
...
open OUT,">$output/$output.assmebled.noninsert.confident.txt" or die $!;


### including the insertion from other chromosome and chromosome:
open CI,">$output/$output.assmebled.insert.blast.txt" or die $!;
...
open UN,">$output/$output.assmebled.unclassified.txt" or die $!;

open OUTA,">$output/$output.assmebled.Ainsertion.fasta" or die $!;

open OUTS,">$output/$output.assmebled.Ainsertion.quality.txt" or die $!;

open OUTU,">$output/$output.assmebled.Anoinsertion.quality.txt" or die $!;
...
foreach my $m (keys %hash){
	...
	if (exists $confInsert1{$m}){
		...
		print OUTS "$m\t$qual{$m}\t$identity\t$match\t$qlength\t$string\n";
		print OUTA ">$m\n$sequence{$m}\n";
	...
	# the left if the length longer than 100bp, we considered them as with insertion	
		...
		print OUTU "$m\t$qual{$m}\tNA\tNA\t$len{$m}\t$string\n";
	}
