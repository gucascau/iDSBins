...
if (!defined $opts{a} || !defined $opts{b} || !defined $opts{o} || !defined $opts{t}|| !defined $opts{c} || !defined $opts{d} || !defined $opts{e}  ) {
	die "************************************************************************
	Usage: $0.pl -a AssembledFasta -b QualityFile -c FinalThreedonorInsertion	-d BLASThit -e BLASTAnnotation -t Sample name -o Output string
				-a Assembled fasta file (Test_detected_assemblysep.Insassembled.fasta)
				-b Reads with Quality results (Test_QC.Allquality.stat)
				-c Final unique three donor insertions (Test_detected_second_threeassembly.uniq.txt, this contain the reads coverage) 
...
while(<B>){
	...
	my ($fcov,$id,$cov,$lengthall,$id2,$chrA,$startA,$endA,$RstartA,$RendA,$lengthA0,$identA,$chrB,$startB,$endB,$RstartB,$RendB,$lengthB0,$identB,$chrC,$startC,$endC,$RstartC,$RendC,$lengthC0,$identC)=split/\t/,$_;
	...
	my $endF=(($lengthall-$endC)<=60)?$endC:($lengthall-50);
	...
	my $ndonor=3;
	if ($startA >60 || ($startB-$endA) >20 ||  ($startC-$endB) >20 || ($lengthall-$endC)>60){
		$ndonor="3orMore";
	...
	my $chrall=join";",($chrA,$chrB,$chrC);
	my $idenall=join ";",($identA,$identB,$identC);
	
	my $quality=$qual{$id};
	print INSSEP ">T$n.A\t$id\n$insertionA\n>T$n.B\t$id\n$insertionB\n>T$n.C\t$id\n$insertionC\n";
	

	print INS ">$id\n$string{$id}\n";
	### print the overall element of insertion #
	
	print OUT "Th$n\t$id\t$sample\t$string{$id}\t$insertion\t$length\t$ndonor\t$idenall\t$fcov\t$quality\t$chrall\tNO\tNO\tNO\tNO\tNO\tNO\tNO\t$upjunction\t$downjunction\n";
	
	### print the first element of insertion #
	
	print OUT "Th$n\.A\t$id\t$sample\t$string{$id}\t$insertionA\t$lengthA\tT1\t$identA\t$fcov\t$quality\t$chrA\t$dstartA\t$dendA\t$strandA\t$EannotationA\t$distA\t$featureA\tNO\tNO\tNO\n";
	
	### print the second element of insertion #
	print OUT "Th$n\.B\t$id\t$sample\t$string{$id}\t$insertionB\t$lengthB\tT2\t$identB\t$fcov\t$quality\t$chrB\t$dstartB\t$dendB\t$strandB\t$EannotationB\t$distB\t$featureB\tNO\tNO\tNO\n";
	
	### print the third element of insertion #
	print OUT "Th$n\.C\t$id\t$sample\t$string{$id}\t$insertionC\t$lengthC\tT3\t$identC\t$fcov\t$quality\t$chrC\t$dstartC\t$dendC\t$strandC\t$EannotationC\t$distC\t$featureC\tNO\tNO\tNO\n";
